...
export default class DeadScreen extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            isLoading: true,
        ...
        if (locationData.isLocationActive == null) {
            ...
            if (this.state.pictureUrl && this.state.QRData) {
                ...
                if (sendImageData.kode == 0) {
                    let getDecaData = await DecaReptilesController.prototype.Dead(
                        this.state.QRData,
                        sendImageData.taregetFile,
                        locationData.latitude,
                        locationData.longitude
                    );
                    console.log(getDecaData);
                    if (getDecaData.Status == 0) {
                        Configuration.newAlert(1, getDecaData.Pesan, 10000, 'bottom');
                        this.restartFrame();
                    } else {
                        Configuration.newAlert(2, getDecaData.Pesan, 10000, 'bottom');
                    }
                } else {
    ...
    render() {
        if (this.state.isLoading) {
            ...
            if (this.state.isCameraActive && !this.state.isQRCodeActive) {
                ...
                return (
                    <Container>
                        <Header {...this.props} iconNameIos='ios-pulse' iconNameAndroid='md-pulse' />
                        <Content>
                            <Card>
                            ...
                            <Col>
                                <Card>
                                    <CardItem header bordered style={[Styles.bgOrange]} >
                                        <Text style={Styles.colorBlack}>Data Hewan Mati</Text>
                                    </CardItem>
                                    ...
                                    <CardItem footer>
                                        <Text>{this.state.QRData != 0 ? 'QRCode: ' + this.state.QRData : ''}</Text>
                                    </CardItem>
                                </Card>
                            </Col>
                        </Content>
