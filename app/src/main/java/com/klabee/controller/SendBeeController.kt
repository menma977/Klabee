...
class SendBeeController : AsyncTask<JSONObject, Void, JSONObject>() {
    override fun doInBackground(vararg params: JSONObject): JSONObject {
        try {
            ...
            val urlParameters = "a=KirimStup&user=${User.username}&client=${params[0]["client"]}" +
                    "&saldo=${params[0]["saldo"]}&lat=${params[0]["lat"]}&long=${params[0]["long"]}" +
                    "&gambar=${params[0]["image"]}&jumlah=${params[0]["count"]}&notrx1=${params[0]["code"]}"
